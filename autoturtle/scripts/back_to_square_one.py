...
class ControlTurtlesim():
    ...
    def __init__(self):
        ...
        rospy.loginfo( "linear (x) velocity = " + str(move_cmd.linear.x))
        # Turn at 0 radians/s
        move_cmd.angular.z = 0
        rospy.loginfo( "angular (z) velocity = " + str(move_cmd.angular.z))
        # Modify this value to cause rotation rad/s
        count = 0

        square_x = input("please enter square x : ") 
        square_y = input("please enter square y : ") 
        tmp_x = square_x
        tmp_y = "None"
        ...
        while not rospy.is_shutdown():
            ...
            if (tmp_x * 10 == move_cmd.linear.x * (count % 100 )):
                move_cmd.linear.x = 0
                move_cmd.angular.z = math.pi / 2
                tmp_x = "None"
                tmp_y = square_y
                count = 0
            elif(math.pi / 2 * 10 ==  move_cmd.angular.z  * (count % 100)):
                move_cmd.linear.x = 1
                move_cmd.angular.z = 0
                count = 0
            elif(tmp_y * 10 == move_cmd.linear.x * (count % 100 )):
                move_cmd.linear.x = 0
                move_cmd.angular.z = math.pi / 2
                tmp_y = "None"
                tmp_x = square_x
